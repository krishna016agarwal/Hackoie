...
import { Layers, UserPlus, Send, Archive, UserCheck, Building2, Loader2, Sparkles } from 'lucide-react';
...
const Profile: React.FC = () => {
    ...
    // 5. Send Join Request to Recommended User
    ...
    // 6. Fetch Team Members (Shared logic for modals)
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-10 py-8 sm:py-12 text-black font-['Inter',sans-serif]">
            ...
            <div className="flex gap-6 sm:gap-12 border-b border-gray-100 mb-8 overflow-x-auto no-scrollbar scroll-smooth px-2">
                {[{ id: 'joined', label: 'Joined', icon: Layers }, { id: 'incoming', label: 'Incoming', icon: UserPlus }, { id: 'sent', label: 'Sent', icon: Send }, { id: 'created', label: 'Created', icon: Archive }, { id: 'friends', label: 'Friends', icon: UserCheck }].map(tab => (
            ...
            <div className="min-h-[400px]">
                ...
                {activeTab === 'incoming' && (
                    <div className="space-y-6">
                        {isLoadingIncoming ? <Loader2 className="animate-spin text-lime-600" size={40} /> : incomingRequests.length === 0 ? (
                            <div className="py-20 text-center bg-gray-50 rounded-[40px] border border-dashed text-gray-400 font-bold uppercase tracking-widest text-xs">No pending applications found</div>
                ...
                {/* --- CREATED SECTION (BLACK CARDS) --- */}
                {activeTab === 'created' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-10">
                        {isLoadingCreated ? <Loader2 className="animate-spin col-span-full mx-auto" size={40} /> : createdTeams.length === 0 ? (
                            <div className="col-span-full py-20 text-center bg-gray-50 rounded-[40px] border border-dashed text-gray-400 font-bold uppercase tracking-widest text-xs">You haven't created any teams yet</div>
            ...
            {showRecModal && (
                <AIRecommendationsModal 
                    ...
                    openMemberProfile={(id: string) => {
                        const u = recommendations.find(r => r._id === id);
